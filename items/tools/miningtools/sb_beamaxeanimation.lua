...
function fillRadius(radius)
  local layer = "overlay"
	...
	if radius == 1 then light({base[1]+0.4,base[2]+0.4},layer) return end
	local t = {}
	for x = 1, radius do
		for y = 1, radius do
			t[#t+1] = {base[1]-(radius/2)+x,base[2]-(radius/2)+y} light(t[#t],layer) --if #t>1 then light(t[#t],layer) end
		end
	end
end

function light(pos,layer)
