...
function init()
  ...
  if status.statusProperty("targetMaterialKind", "") ~= "organic" then
    effect.expire()
  end --glitch and novas have red particles when hit. blood?? i guess this can also be internal bleeding. also this doesnt work for monsters??
...
--  if spurt > 0 then status.modifyResource("health",-(spurt/10)) spurt=spurt-1 end
  ...
  --Reduce duration when healing
  if lastHealth < status.resource("health") then
    effect.modifyDuration(-0.3)
  end
  lastHealth = status.resource("health")
end