...
function apply(input)
  ...
  if (string.lower(category) == config.getParameter("sb_copybookTypes",defaults)[1])
    or output:instanceValue(config.getParameter("sb_copybooktypes",defaults)[2],false)
    or output.name == config.getParameter("sb_copybooktypes",defaults)[3]
  then
    if output.count < maxStack then exchange = 1 end
    output.count = output.count + exchange
  end
