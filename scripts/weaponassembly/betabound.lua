local sbisValidWeapon = isValidWeapon or function() end
local sbbuildShortDescription = buildShortDescription or function() return "Weapon" end
...
function buildShortDescription(partName, weaponConfig) --I'm sure it's fine right now, but shouldn't we be making use of what we're replacing?
  ...
  --local name = string.gsub(string.format("%s%s",weaponConfig.shortdescription,string.gsub(sbbuildShortDescription(partName, weaponConfig), "(%w+)","",1)),"  "," ")
  --surely there's a better way to do this. this loops i*3 times!
