...
function init()
  if entity.entityType() ~= "player" and status.statusProperty("sb_outpostnpcspawner") ~= true then
    ...
    for i = 1, #npcs do
      if not npcsToSkip[npcs[i][2]] then
        world.spawnNpc(positions[i], table.unpack(npcs[i]))
      end
    end
    ...
    status.setStatusProperty("sb_outpostnpcspawner", true)
  end
end

function update()
  if positions and world.universeFlagSet("sb_hylotlwarriorE2") then
    if world.spawnVehicle("sb_kennel", positions[13]) then
